...
const getWelcomeEmailTemplate = (firstName: string) => getEmailWrapper(`
    ...
    <p style="color: #64748b; font-size: 14px; text-align: center; margin-top: 30px;">
        Need help? Contact us at XXXXXXX
    </p>
`);
...
const getInvoiceEmailTemplate = (data: InvoiceData) => {
    ...
    return getEmailWrapper(`
        ...
        <p style="color: #64748b; font-size: 14px; text-align: center; margin-top: 30px;">
            Questions about your order? Contact us at XXXXXXX
        </p>
    `);
};
...
const getLiveClassEmailTemplate = (data: LiveClassData) => getEmailWrapper(`
    ...
    <p style="color: #64748b; font-size: 14px; text-align: center; margin-top: 30px;">
        Questions? Contact us at XXXXXXX
    </p>
`);

// ===================================================================
// BOOKING PAYMENT RECEIPT / INVOICE  (Phase E)
// ===================================================================

// Small money formatter mirroring the PDF (e.g. "Tk 12,500").
const fmtMoney = (value: unknown, currency?: string): string => {
    const c = (currency || 'BDT').toUpperCase();
    const symbol =
        c === 'BDT' ? 'Tk' : c === 'USD' ? '$' : c === 'EUR' ? '€' : c === 'GBP' ? '£' : c;
    const n = Number(value) || 0;
    return `${symbol} ${n.toLocaleString('en-US', { maximumFractionDigits: 2 })}`;
};

const bookingTypeLabel = (type?: string): string => {
    const map: Record<string, string> = {
        tour: 'Tour',
        hajj: 'Hajj / Umrah',
        study: 'Study',
    };
    return map[(type || '').toLowerCase()] || (type ? type : 'Service');
};

// Customer-facing "Payment Receipt / Invoice" email.
const getBookingReceiptTemplate = (
    booking: IBooking,
    lastPaymentAmount: number
) => {
    const trackingId = booking.trackingId || String((booking as { _id?: unknown })._id || '');
    const trackUrl = `${config.frontend_url}/track`;
    const currency = booking.currency || 'BDT';

    return getEmailWrapper(`
        <div style="text-align: center; margin-bottom: 30px;">
            <div style="width: 80px; height: 80px; background: linear-gradient(135deg, #3590CF 0%, #0a1628 100%); border-radius: 50%; margin: 0 auto 20px; display: flex; align-items: center; justify-content: center;">
                <span style="font-size: 36px;">🧾</span>
            </div>
            <h2 style="color: #1e293b; font-size: 24px; margin-bottom: 10px;">Payment Received</h2>
            <p style="color: #64748b; font-size: 16px;">Your invoice is attached to this email</p>
        </div>

        <p style="color: #334155; font-size: 16px; line-height: 1.6; margin-bottom: 20px;">
            Hi <strong>${booking.name || 'there'}</strong>,
        </p>

        <p style="color: #334155; font-size: 16px; line-height: 1.6; margin-bottom: 25px;">
            We have received your payment of
            <strong style="color: #EF8C2C;">${fmtMoney(lastPaymentAmount, currency)}</strong>
            for your <strong>${bookingTypeLabel(booking.type)}</strong> service. A detailed
            PDF invoice is attached to this email for your records.
        </p>

        <!-- Summary box -->
        <div style="background: #f8fafc; border-radius: 12px; padding: 24px; margin-bottom: 25px; border: 1px solid #e2e8f0;">
            <table style="width: 100%; border-collapse: collapse; font-size: 14px;">
                <tr>
                    <td style="padding: 8px 0; color: #64748b;">Service</td>
                    <td style="padding: 8px 0; text-align: right; color: #1e293b; font-weight: 600;">${booking.serviceName || bookingTypeLabel(booking.type)}</td>
                </tr>
                <tr>
                    <td style="padding: 8px 0; color: #64748b;">Tracking ID</td>
                    <td style="padding: 8px 0; text-align: right; color: #3590CF; font-weight: 700;">${trackingId}</td>
                </tr>
                <tr style="border-top: 1px solid #e2e8f0;">
                    <td style="padding: 8px 0; color: #64748b;">Paid this time</td>
                    <td style="padding: 8px 0; text-align: right; color: #1e8f4e; font-weight: 700;">${fmtMoney(lastPaymentAmount, currency)}</td>
                </tr>
                <tr>
                    <td style="padding: 8px 0; color: #64748b;">Total paid</td>
                    <td style="padding: 8px 0; text-align: right; color: #1e293b; font-weight: 600;">${fmtMoney(booking.paidAmount, currency)}</td>
                </tr>
                <tr>
                    <td style="padding: 8px 0; color: #64748b;">Total amount</td>
                    <td style="padding: 8px 0; text-align: right; color: #1e293b; font-weight: 600;">${fmtMoney(booking.amount, currency)}</td>
                </tr>
                <tr style="border-top: 2px solid #e2e8f0;">
                    <td style="padding: 10px 0 0; color: #0a1628; font-weight: 700;">Amount due</td>
                    <td style="padding: 10px 0 0; text-align: right; font-weight: 800; font-size: 16px; color: ${(Number(booking.dueAmount) || 0) > 0 ? '#EF8C2C' : '#1e8f4e'};">${fmtMoney(booking.dueAmount, currency)}</td>
                </tr>
            </table>
        </div>

        <div style="text-align: center; margin-top: 30px;">
            <a href="${trackUrl}" style="display: inline-block; background: linear-gradient(135deg, #3590CF 0%, #0a1628 100%); color: white; text-decoration: none; padding: 14px 40px; border-radius: 8px; font-weight: 600; font-size: 16px;">
        ...
        <p style="color: #64748b; font-size: 14px; text-align: center; margin-top: 24px;">
            ...
            <a href="${trackUrl}" style="color: #3590CF;">${config.frontend_url}/track</a>
...
const EmailService = {
    ...
    // Send booking payment receipt + PDF invoice to the customer,
    // plus a short notification copy to the admin. (Phase E)
    async sendBookingInvoiceEmail(
        booking: IBooking,
        pdfBuffer: Buffer,
        ...
        try {
            const trackingId =
                booking.trackingId || String((booking as { _id?: unknown })._id || '');
            const currency = booking.currency || 'BDT';
            ...
            // 2) Admin notification copy (best-effort)
            const adminEmail = settings?.contactEmail || config.email.from;
            if (adminEmail) {
                ...
                await transporter.sendMail({
                    ...
                    html: getEmailWrapper(`
                        <h2 style="color:#0a1628;font-size:20px;margin-bottom:16px;">New payment recorded</h2>
                        <table style="width:100%;border-collapse:collapse;font-size:14px;">
                            <tr><td style="padding:6px 0;color:#64748b;">Customer</td><td style="padding:6px 0;text-align:right;font-weight:600;">${booking.name || '—'}</td></tr>
                            <tr><td style="padding:6px 0;color:#64748b;">Email</td><td style="padding:6px 0;text-align:right;">${booking.email || '—'}</td></tr>
                            <tr><td style="padding:6px 0;color:#64748b;">Phone</td><td style="padding:6px 0;text-align:right;">${booking.phone || '—'}</td></tr>
                            <tr><td style="padding:6px 0;color:#64748b;">Service</td><td style="padding:6px 0;text-align:right;font-weight:600;">${booking.serviceName || bookingTypeLabel(booking.type)}</td></tr>
                            <tr><td style="padding:6px 0;color:#64748b;">Tracking ID</td><td style="padding:6px 0;text-align:right;color:#3590CF;font-weight:700;">${trackingId}</td></tr>
                            <tr style="border-top:1px solid #e2e8f0;"><td style="padding:8px 0;color:#64748b;">Paid this time</td><td style="padding:8px 0;text-align:right;color:#1e8f4e;font-weight:700;">${fmtMoney(amountThisTime, currency)}</td></tr>
                            <tr><td style="padding:6px 0;color:#64748b;">Total paid</td><td style="padding:6px 0;text-align:right;font-weight:600;">${fmtMoney(booking.paidAmount, currency)}</td></tr>
                            <tr><td style="padding:6px 0;color:#64748b;">Amount due</td><td style="padding:6px 0;text-align:right;font-weight:700;color:${dueOutstanding ? '#EF8C2C' : '#1e8f4e'};">${fmtMoney(booking.dueAmount, currency)}</td></tr>
                        </table>
                    `),
                    attachments,
                });
